...
def import_geonames_file(data, timezones=None):
    """
    ...
    ...     '2633442\\tWyre\\tWyre\\tViera\\t59.1166667\\t-2.9666667\\tT\\tISL\\tGB\\tGB\\tV9\\t\\t\\t\\t0\\t\\t1\\tEurope/London\\t2004-09-24',
    ...     '2633443\\tWraysbury\\tWraysbury\\tWyrardisbury\\t51.45\\t-0.55\\tP\\tPPL\\tGB\\t\\tP9\\t\\t\\t\\t0\\t\\t28\\tEurope/London\\t2006-08-21']))
    >>> locations = import_geonames_file(locations_file)
    ...
    @type timezones: C{dict}
    @param timezones: Dictionary of timezone names and UTC offsets in the
        format generated by C{import_timezones_file}
    @rtype: C{dict}
    ...
    for line in data:
        ...
        for pos, elem in zip(range(19), chunk):
            ...
            elif pos == 17 and not elem == "" and timezones:
                timezone = timezones[elem][0]
            elif pos == 18:
        ...
        locations[chunk[0]] = Location(*chunk)
    return locations

def import_timezones_file(data):
    """
    ...
    ...     'Asia/Dubai\\t4.0\\t4.0',
    ...     'Asia/Kabul\\t4.5\\t4.5']))
    >>> timezones = import_timezones_file(timezones_file)
